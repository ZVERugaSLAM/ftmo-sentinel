...
if "GEMINI_API_KEY" in st.secrets:
    ...
    # Використовуємо пряму назву. Якщо flash недоступна в регіоні хмари, вмикається pro.
    try:
...
with tab3:
    ...
    # ПЕРШИЙ РЯДОК: Ліквідність та Кредитний ризик
    row1_1, row1_2, row1_3 = st.columns(3)
    
    with row1_1:
        st.metric("10Y-2Y Yield Spread", "+0.60%", delta="Un-inversion", delta_color="inverse")
        st.help("Різниця дохідності 10- та 2-річних облігацій США. Перехід від від'ємного (інверсія) до додатного значення історично збігається з початком рецесії через різке зниження ставок ФРС.")
        
    with row1_2:
        st.metric("US Reverse Repo (RRP)", "$0.5B", delta="Critical Drain", delta_color="inverse")
        st.help("Об'єм надлишкової ліквідності банків у ФРС. Падіння до нуля означає повне виснаження 'подушки безпеки'. Ринки стають максимально вразливими до шоків ліквідності.")
        
    with row1_3:
        st.metric("US High Yield Spread", "2.86%", delta="Low Risk", delta_color="normal")
        st.help("Спред між 'сміттєвими' корпоративними облігаціями та держоблігаціями США (ICE BofA OAS). Ріст вище 5.00% означає паніку на кредитному ринку та ризик масових корпоративних дефолтів.")

    # ДРУГИЙ РЯДОК: Ринок праці, Волатильність та Настрій
    row2_1, row2_2, row2_3 = st.columns(3)
    
    with row2_1:
        st.metric("Sahm Rule Indicator", "0.30%", delta="Rising", delta_color="inverse")
        st.help("Індикатор рецесії за правилом Самм. Якщо показник досягає 0.50%, економіка США де-факто увійшла в рецесію. Поточний тренд — висхідний.")
        
    with row2_2:
        st.metric("Job Search 'Find a Job'", "+12%", delta="High Risk", delta_color="inverse")
        st.help("Динаміка запитів у Google Trends. Випереджальний соціальний індикатор безробіття, який сигналізує про проблеми до виходу офіційних звітів NFP.")

    with row2_3:
        st.metric("VIX (Fear Index)", "21.60", delta="Elevated", delta_color="inverse")
        st.help("Індекс очікуваної волатильності S&P 500. Значення вище 20-25 вказують на зростання страху та хеджування ризиків великим капіталом.")
    ...
    if st.button("Згенерувати актуальний звіт 'Великої п'ятірки'"):
        with st.spinner("Аналіз кривої дохідності, RRP, HY Spread та ринку праці..."):
            report_prompt = """
            Проаналізуй поточну фазу світового ринку на основі 5 індикаторів:
            1) Де-інверсія кривої дохідності (+0.60%).
            2) Повне виснаження Reverse Repo ліквідності ($0.5B).
            3) High Yield Spread на рівні 2.86%.
            4) Sahm Rule наближається до рецесійного порогу (0.30%).
            5) Ріст запитів пошуку роботи (+12%).
            Зроби жорсткий висновок щодо ризику обвалу, впливу на DXY та XAUUSD. Аналіз має бути аргументованим та без води.
            """
            report = get_sentinel_analysis("Global Liquidity & Macro Risk", report_prompt)
            st.markdown(report)
